...
const Navbar: React.FC = () => {
  const navItems = [
    ...
    { label: 'Blog', icon: <FileText size={20} />, href: '#blog' },
  ...
  return (
    <nav className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50">
      <motion.div
        ...
        className="glass rounded-full px-4 py-3 flex items-center gap-1 md:gap-4 shadow-2xl"
      >
        {navItems.map((item, idx) => (
          <motion.a
            ...
            className="flex items-center justify-center w-10 h-10 md:w-12 md:h-12 rounded-full text-[var(--text-muted)] hover:text-cyan-500 hover:bg-cyan-500/10 transition-colors"
            title={item.label}
          >
            {item.icon}
          </motion.a>
