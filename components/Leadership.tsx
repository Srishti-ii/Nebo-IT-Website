...
const LeadershipCard = ({ person }: { person: typeof LEADERSHIP_DATA[0] }) => {
  ...
  return (
    <div 
      ...
      className={`relative bg-[var(--bg-secondary)]/60 rounded-xl overflow-hidden shadow-lg transition-all duration-500 cursor-pointer border border-[var(--border-subtle)] hover:border-blue-500/50 flex flex-col ${
    ...
    >
      <div className="p-4 flex flex-col h-full">
        ...
        <div className={`transition-all duration-500 ease-in-out ${isExpanded ? 'ml-0 mt-28' : 'ml-[35%]'}`}>
          <h3 className={`var(--text-main) leading-tight font-extrabold transition-all duration-500 ${isExpanded ? 'text-lg mb-1' : 'text-sm mb-1'}`}>
            {person.name}
          </h3>
        ...
        <div className={`absolute top-0 transition-all duration-500 ease-in-out overflow-hidden ${
          isExpanded 
            ? 'left-4 top-4 w-24 h-24 rounded-lg border border-[var(--border-subtle)]' 
            : 'left-0 w-1/3 h-full rounded-none border-r border-[var(--border-subtle)]'
        }`}>
          <img 
            src={person.image} 
            alt={person.name} 
        ...
        <div className={`transition-all duration-500 ease-in-out flex-grow ${isExpanded ? 'mt-2 ml-0' : 'ml-[35%] mt-0'}`}>
          <p className={`text-[var(--text-muted)] leading-relaxed transition-all duration-500 ${
  ...
  return (
    <section className="py-20 px-6">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-2xl font-bold var(--text-main) text-center mb-12 uppercase tracking-widest">
          PATRONS
        </h2>
